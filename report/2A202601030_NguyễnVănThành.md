    ...
    | Khóa/Lớp | K4 |
    | Tên nhóm | T-Hexa |
    | Vai trò chính | Lead — Role 1: Pipeline Integration & Release |
    ...
    | --- | --- | --- | --- | --- |
    | Phần việc chính | `src/core/`, `src/pipelines/phase1.py`, `src/pipelines/corruption_flow.py` | Artifact/module từ bước trước | Module và artifact đúng contract | Hoàn thành |
    ...
    Tôi chịu trách nhiệm chính về điều phối toàn bộ baseline và corruption flow, quản lý contract, prerequisite, artifact paths, Git integration và release. Ownership trong báo cáo này chỉ áp dụng cho phạm vi role đã được nhóm phân công; kết quả end-to-end là kết quả tích hợp chung của cả nhóm.
    ...
    | --- | --- | --- | --- |
    | Hoàn thiện phần việc của role | `src/core/`, `src/pipelines/phase1.py`, `src/pipelines/corruption_flow.py` | Hai entrypoint chạy end-to-end, tạo metrics, answers, quality/freshness và comparison reports. | Chạy baseline và corruption flow |
    ...
    **Output cụ thể:** Hai entrypoint chạy end-to-end, tạo metrics, answers, quality/freshness và comparison reports.
    ...
    | Output | Hai entrypoint chạy end-to-end, tạo metrics, answers, quality/freshness và comparison reports. |
    ...
    - **Phương án đã chọn:** Giữ test set, `top_k` và cấu hình retrieval cố định giữa ba trạng thái; tách collection/artifact baseline, corrupted và repaired.
    ...
    - **Triệu chứng/blocker:** Python mặc định là 3.14 và nhánh tích hợp có conflict ở `phase1.py`. Chọn Python 3.12, xử lý merge conflict, sau đó chạy lại cả hai entrypoint.
