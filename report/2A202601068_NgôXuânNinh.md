    ...
    | --- | --- | --- | --- | --- |
    | Phần việc chính | `src/retrieval/` và `data/embeddings/` | Artifact/module từ bước trước | Module và artifact đúng contract | Hoàn thành |
    ...
    Tôi chịu trách nhiệm chính về MiniLM embeddings, ChromaDB persistent collections, semantic search, exact lookup, QA và agent tools. Ownership trong báo cáo này chỉ áp dụng cho phạm vi role đã được nhóm phân công; kết quả end-to-end là kết quả tích hợp chung của cả nhóm.
    ...
    | --- | --- | --- | --- |
    | Hoàn thiện phần việc của role | `src/retrieval/` và `data/embeddings/` | Ba Chroma collections/manifests, search/lookup hoạt động và retrieval hit rate baseline đạt 1.0. | Chạy baseline và corruption flow |
    ...
    **Output cụ thể:** Ba Chroma collections/manifests, search/lookup hoạt động và retrieval hit rate baseline đạt 1.0.
    ...
    | Output | Ba Chroma collections/manifests, search/lookup hoạt động và retrieval hit rate baseline đạt 1.0. |
    ...
    - **Phương án đã chọn:** Kết hợp exact-title lookup với semantic top-k; sử dụng collection riêng cho baseline, corrupted và repaired để không ghi đè.
    ...
    - **Triệu chứng/blocker:** Agent demo thiếu `GOOGLE_API_KEY`. Thiết kế pipeline cho phép skip agent demo có kiểm soát trong khi retrieval và deterministic QA vẫn chạy.
