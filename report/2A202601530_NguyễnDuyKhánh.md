...
| --- | --- | --- | --- | --- |
| Phần việc chính | `src/ingestion/cleaning.py`, `src/ingestion/corruption.py` | Artifact/module từ bước trước | Module và artifact đúng contract | Hoàn thành |
...
Tôi chịu trách nhiệm chính về chuẩn hóa clean schema, dedupe, tính `age_days`, tạo `text_for_embedding`, mô phỏng corruption và hỗ trợ repair từ raw. Ownership trong báo cáo này chỉ áp dụng cho phạm vi role đã được nhóm phân công; kết quả end-to-end là kết quả tích hợp chung của cả nhóm.
...
| --- | --- | --- | --- |
| Hoàn thiện phần việc của role | `src/ingestion/cleaning.py`, `src/ingestion/corruption.py` | Clean 24 rows, corrupted 22 rows, repaired 24 rows; corruption log dùng seed 1337. | Chạy baseline và corruption flow |
...
**Output cụ thể:** Clean 24 rows, corrupted 22 rows, repaired 24 rows; corruption log dùng seed 1337.
...
| Output | Clean 24 rows, corrupted 22 rows, repaired 24 rows; corruption log dùng seed 1337. |
...
- **Phương án đã chọn:** Repair bằng cách chạy lại cleaning từ raw snapshot bất biến thay vì chỉnh corrupted DataFrame hoặc copy baseline outputs.
...
- **Triệu chứng/blocker:** Sau corruption phải rebuild `text_for_embedding` và `age_days`; nếu không, vector index và freshness không phản ánh dữ liệu lỗi. Hàm corruption cập nhật lại cả hai.
