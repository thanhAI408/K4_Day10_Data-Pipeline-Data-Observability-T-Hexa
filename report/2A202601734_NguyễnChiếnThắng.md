    ...
    | --- | --- | --- | --- | --- |
    | Phần việc chính | `src/evaluation/`, `src/observability/`, `data/eval/`, `data/quality/`, `data/reports/` | Artifact/module từ bước trước | Module và artifact đúng contract | Hoàn thành |
    ...
    Tôi chịu trách nhiệm chính về tạo fixed test set, tính retrieval/answer metrics, quality checks, freshness reports và comparison reports. Ownership trong báo cáo này chỉ áp dụng cho phạm vi role đã được nhóm phân công; kết quả end-to-end là kết quả tích hợp chung của cả nhóm.
    ...
    | --- | --- | --- | --- |
    | Hoàn thiện phần việc của role | `src/evaluation/`, `src/observability/`, `data/eval/`, `data/quality/`, `data/reports/` | 18-question test set; baseline/corrupted/repaired metrics; quality và freshness reports; Markdown comparison. | Chạy baseline và corruption flow |
    ...
    **Output cụ thể:** 18-question test set; baseline/corrupted/repaired metrics; quality và freshness reports; Markdown comparison.
    ...
    | Output | 18-question test set; baseline/corrupted/repaired metrics; quality và freshness reports; Markdown comparison. |
    ...
    - **Phương án đã chọn:** Dùng cùng 18-question test set cho ba trạng thái và fallback heuristic judge khi LLM evaluator không khả dụng.
    ...
    - **Triệu chứng/blocker:** Ragas chưa bật và LLM judge có thể không có credential. Ghi rõ trạng thái skipped/fallback thay vì báo thành công giả.
